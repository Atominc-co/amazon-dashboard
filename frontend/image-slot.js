...
(() => {
  ...
  class ImageSlot extends HTMLElement {
    ...
    _render() {
      ...
      // An early/pre-hydration render pass can momentarily set `src` to the
      // literal unresolved template text (e.g. "{{ p.imageUrl }}") before
      // real data replaces it. That string is not a URL — treat it as
      // absent so the browser never issues a real fetch for it (was seen
      // as a spurious 404 in Network).
